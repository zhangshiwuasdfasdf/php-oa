...
// 		$this -> assign('id', $id);
	...
	function export_monthly_report(){
		...
		$q = $q -> setCellValue("A14", '下月工作计划（A类最重要 B类重要 C类次重要）');
		...
		// Rename worksheet
		$title = '月报';
		$objPHPExcel -> getActiveSheet() -> setTitle('月报');
	...
	function import_monthly_report(){
		...
		if ($opmode == "import") {
			...
			if (!$upload -> upload()) {
				...
				$model_monthly_report = M("MonthlyReport");
				$monthly_report = array();
				...
				$monthly_report['user_id'] = get_user_id();
				$monthly_report['user_name'] = get_user_name();
				$monthly_report['dept_id'] = get_dept_id();
				$monthly_report['dept_name'] = get_dept_name();
				$monthly_report['create_time'] = time();
				$monthly_report['content'] = $sheetData[$ii*2]['B'];
				$monthly_report['plan'] = $sheetData[$ii*2+$kk+7]['B'];
				$monthly_report['is_del'] = 0;
				$monthly_report['is_submit'] = 0;
				$monthly_report['work_date'] = date('Y-m', strtotime('0 month'));
	
				$pid = $model_monthly_report -> add($monthly_report);
	
				$model_monthly_report_detail = M("MonthlyReportDetail");
				...
				for ($jj=1;$jj<$ii;$jj++){
					...
					$model_monthly_report_detail -> add($data_detail);
				...
				$this -> assign('jumpUrl', U("monthly_report/edit",array('id'=>$pid)));
				$this -> success('导入成功！');
			}
		} else {
			$this -> display();
	...
	function json() {
		...
		$list = M("MonthlyReport") -> where($where) -> order('work_date desc') -> select();
		foreach ($list as $k=>$v){
